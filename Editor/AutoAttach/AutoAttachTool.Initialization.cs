    ...
    internal partial class AutoAttachTool
    {
        ...
        private static void OnCompile()
        {
            var allTypes = AppDomain.CurrentDomain.GetAssemblies().SelectMany(assembly => assembly.GetTypes()
                .Where(type => type.IsInstantiable() && !type.IsEnum && !type.IsPrimitive)).ToArray();

            Dictionary<Type, List<FieldData>> cache =
                new Dictionary<Type, List<FieldData>>();


            Setters.AddRange(allTypes.Where(type => type.ImplementsOrInherits(typeof(SetterBase)))
                .Select(type => (SetterBase)Activator.CreateInstance(type)));
        ...
        private static bool Fill(IEnumerable<FieldInfo> buffer, Type context, FieldInfo fieldInfo,
            List<FieldData> values)
        {
            if (!fieldInfo.FieldType.IsValueType || TryFindSetter(fieldInfo.FieldType, out _))
            {
                try
                {
        ...
        private static void Fill(ISet<FieldInfo> buffer, Type type, List<FieldData> values, int depth)
        {
                                                           ...
                                                           BindingFlags.NonPublic))
            {
                ...
                if (!Fill(buffer, type, fieldInfo, values) && depth + 1 < MaxDepth &&
                    !fieldInfo.FieldType.ImplementsOrInherits(typeof(Object)))
